...
const COMMANDS_WITH_SUBCOMMANDS = new Set(['pr', 'issue', 'repo', 'release', 'run', 'workflow', 'gist'])
...
function evaluatePrPolicy(subcommand: string | null, positionalArgs: string[], policy: GitHubPolicy): PolicyDecision {
 ...
 */
function checkOwnedPr(positionalArgs: string[], policy: GitHubPolicy, subcommand: string): PolicyDecision {
...
async function apiPrView(parsed: ParsedGhCommand, policy: GitHubPolicy, token: string): Promise<void> {
...
async function apiPrEdit(parsed: ParsedGhCommand, policy: GitHubPolicy, token: string): Promise<void> {
  const prNumber = parsed.positionalArgs[0] ?? policy.ownedPrNumber
  if (!prNumber) {
    process.stderr.write('[bouncer:gh] error: PR number required\n')
    process.exit(1)
...
async function apiIssueView(parsed: ParsedGhCommand, policy: GitHubPolicy, token: string): Promise<void> {
...
async function apiDirect(parsed: ParsedGhCommand, policy: GitHubPolicy, token: string): Promise<void> {
