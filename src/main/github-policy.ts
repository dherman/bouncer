...
export async function installGhShim(
  ...
  // cd to the shim's source directory so relative imports (./types.js etc.)
  // resolve correctly regardless of the caller's cwd (which is the worktree).
  const shimSourceDir = ghShimTsPath.substring(0, ghShimTsPath.lastIndexOf("/"));
...
cd "${shimSourceDir}" && exec "${nodePath}" --import "${tsxEsmPath}" "${ghShimTsPath}" "$@"
`;
