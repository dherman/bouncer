...
export class WorktreeManager {
  ...
  async create(sessionId: string, projectDir: string): Promise<WorktreeInfo> {
    ...
    await execFileAsync('git', ['worktree', 'add', '-b', branch, '--', worktreePath, 'HEAD'], { cwd: projectDir })
    ...
    if (resolvedCommonDir.startsWith(expectedPrefix)) {
      ...
      console.warn(`Git common dir ${resolvedCommonDir} is outside project ${projectDir} — skipping sandbox grant`)
    }
  ...
  async remove(info: WorktreeInfo): Promise<void> {
    try {
      await execFileAsync('git', ['worktree', 'remove', '--force', '--', info.path], { cwd: info.projectDir })
    } catch (err) {
