...
export async function ensureAgentImage(): Promise<string> {
  ...
  await execFileAsync('docker', ['build', '-t', imageTag, '-f', dockerfilePath, dockerDir], { timeout: 600_000 })
...
export function buildDockerRunArgs(config: ContainerConfig): string[] {
  ...
  for (const m of config.mounts) {
    const flag = m.readOnly ? `${m.hostPath}:${m.containerPath}:ro` : `${m.hostPath}:${m.containerPath}`
    args.push('-v', flag)
  }
...
export async function cleanupOrphanContainers(activeSessionIds: Set<string>): Promise<void> {
  ...
  try {
    const result = await execFileAsync(
      'docker',
      ['ps', '-a', '--filter', 'label=bouncer.managed=true', '--format', '{{.Label "bouncer.sessionId"}}\t{{.Names}}'],
      { timeout: 10_000 },
    )
