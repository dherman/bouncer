...
export class WorkspaceManager {
  ...
  async createWorkspace(
    ...
    const resolvedPolicyId =
      agentType === 'claude-code' || agentType === 'replay' ? (policyId ?? this.policyRegistry.defaultId) : null
  ...
  private async initializeWorkspace(
    ...
    try {
      ...
      if (dockerAvailable) {
        ...
        if (agentType === 'echo') {
          ...
          const shimBundlePath =
            template.github && workspace.githubPolicy ? join(POLICY_DIR, 'gh-shim-bundle.js') : undefined
          ...
          if (template.network.access === 'filtered') {
            ...
            const inspectedDomains = template.network.access === 'filtered' ? template.network.inspectedDomains : []
      ...
      if (containerConfig) {
        ...
        const { cmd, args, env, cwd } = resolveAgentCommand(agentType, workingDir, sandboxConfig, worktree?.path)
      ...
      agentProcess.on('exit', (code) => {
        ...
        if (workspace.status !== 'closed') {
          const errorMessage =
            workspace.status === 'initializing' ? collectedStderr.trim() || `Agent exited with code ${code}` : undefined
        ...
        (_agent) => ({
          async sessionUpdate(params) {
            ...
            if (update.sessionUpdate === 'agent_message_chunk' && update.content.type === 'text') {
              ...
              if (agentMsg) {
                const meta = update._meta as { claudeCode?: { toolName?: string; toolResponse?: unknown } } | undefined
                const rawInput =
  ...
  async closeWorkspace(workspaceId: string): Promise<void> {
    ...
    if (workspace.sessionNetwork) {
      await workspace.sessionNetwork
        .cleanup()
        .catch((err) => console.warn(`Failed to remove network for workspace ${workspaceId}:`, err))
    }
  ...
  async closeAllWorkspaces(): Promise<void> {
    const activeWorkspaces = Array.from(this.workspaces.values()).filter((s) => s.status !== 'closed')
  ...
  async cleanupOrphans(): Promise<void> {
    ...
    await cleanupOrphanContainers(activeIds).catch((err) => console.warn('Failed to clean up orphan containers:', err))
    await cleanupOrphanNetworks(activeIds).catch((err) => console.warn('Failed to clean up orphan networks:', err))
