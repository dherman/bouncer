...
import { evaluateGitHubRequest, parseGitReceivePack, evaluateGitPush } from './github-policy-engine.js'
...
function forwardWithCapture(
  ...
  const proxyReq = https.request(options, (proxyRes) => {
    ...
    proxyRes.on('end', () => {
      ...
      if (!captureLimitExceeded && proxyRes.statusCode && proxyRes.statusCode >= 200 && proxyRes.statusCode < 300) {
