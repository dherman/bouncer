...
function App() {
  ...
  const handleUpdate = useCallback(
    (update: SessionUpdate) => {
      switch (update.type) {
        case 'status-change':
          setSessions((prev) =>
            prev.map((s) => (s.id === update.sessionId ? { ...s, status: update.status } : s)),
          );
          ...
          setMessagesBySession((prevMsgs) => {
            ...
            if (msgs) {
              next.set(
                update.sessionId,
                msgs.map((m) =>
                  m.id === update.messageId ? { ...m, text: finalText, streaming: false } : m,
                ),
              );
            }
            return next;
  ...
  const activeMessages = activeSessionId ? (messagesBySession.get(activeSessionId) ?? []) : [];
  const activeViolations = activeSessionId ? (violationsBySession.get(activeSessionId) ?? []) : [];
