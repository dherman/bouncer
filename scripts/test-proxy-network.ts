...
await test("networkMode 'proxy' without networkName throws", () => {
  assert.throws(() => buildDockerRunArgs(makeContainerConfig({ networkMode: 'proxy' })), /networkName is required/)
})
  ...
  try {
    ...
    await test('container on session network routes traffic through proxy', async () => {
      ...
      try {
        ...
        assert.equal(allowedResult.trim(), '200', `allowed domain should return 200, got ${allowedResult.trim()}`)
