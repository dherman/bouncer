...
async function resolveAgentImage(): Promise<string> {
  ...
  try {
    ...
    await execFileAsync('docker', ['build', '-t', imageTag, '-f', dockerfilePath, join(process.cwd(), 'docker')], {
      timeout: 600_000,
    })
    return imageTag
  }
...
try {
  ...
  await test("CA trust: container's curl trusts Bouncer CA (entrypoint installs it)", async () => {
    ...
    assert.ok(/^[0-9]{3}$/.test(stdout.trim()), `curl should trust the Bouncer CA (got: ${stdout.trim()})`)
  })
  ...
  await test('teardown: proxy stops and network is removed cleanly', async () => {
    ...
    try {
      ...
      assert.ok(msg.includes('not found') || msg.includes('No such network'), `expected network-not-found, got: ${msg}`)
    }
  })
} finally {
