...
try {
  ...
  const dockerRunBase = [
    ...
    'no_proxy=localhost,127.0.0.1,::1',
    '-e',
    `BOUNCER_PROXY_HOST=host.docker.internal`,
    '-e',
    `BOUNCER_PROXY_PORT=${proxy.port}`,
    '-v',
  ...
  await test('node https.request to github.com goes through proxy', async () => {
    ...
    if (stderr)
      console.log(
        `    stderr (last 5 lines): ${stderr.trim().split('\n').slice(-5).join('\n      ')}`,
      );
    assert.ok(
      stdout.includes('status:200') ||
        stdout.includes('status:301') ||
        stdout.includes('status:302'),
  ...
  await test('npm install electron succeeds in sandbox', async () => {
    ...
    assert.ok(
      !stdout.includes('ETIMEDOUT') &&
        !stdout.includes('ECONNREFUSED') &&
        stdout.includes('npm install exit code: 0'),
      `npm install failed`,
    );
  });
