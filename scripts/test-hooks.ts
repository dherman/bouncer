...
import { hooksDir, allowedRefsPath, generatePrePushHook, installHooks, cleanupHooks } from '../src/main/hooks.js'
...
await test('push to denied ref fails', async () => {
  ...
  try {
    ...
    await execFileAsync('git', ['-C', repo.worktreeDir, 'push', 'origin', `HEAD:refs/heads/unauthorized-branch`])
    assert.fail('push to unauthorized branch should have been denied')
  } catch (err: unknown) {
    const stderr = (err as { stderr?: string }).stderr ?? ''
    assert.ok(stderr.includes('[bouncer:git] DENY'), `expected bouncer denial message, got: ${stderr}`)
  }
})
