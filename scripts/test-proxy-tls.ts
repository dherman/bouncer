...
import { ensureCA, generateHostCert, clearHostCertCache, type BouncerCA } from '../src/main/proxy-tls.js'
...
await test('TLS chain verification: tls.connect trusts host cert when CA is provided', async () => {
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      const socket = tls.connect({ host: '127.0.0.1', port, ca: ca.cert, servername: hostname }, () => {
    ...
    await new Promise<void>((resolve, reject) => {
      const socket = tls.connect({ host: '127.0.0.1', port, servername: hostname, rejectUnauthorized: true }, () => {
