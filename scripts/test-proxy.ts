...
function httpViaProxy(proxyPort: number, targetUrl: string): Promise<{ status: number; body: string }> {
...
await test("MITM'd domain: client sees cert signed by Bouncer CA", async () => {
  ...
  const httpsUpstream = https.createServer({ cert: upstreamCert.cert, key: upstreamCert.key }, (_req, res) => {
    res.writeHead(200)
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      tlsSocket.on('secureConnect', () => {
        ...
        assert.ok(peerCert.issuer.includes('Bouncer Proxy CA'), 'cert should be issued by Bouncer CA')
...
await test("MITM'd domain with onMitmRequest handler: handler is called", async () => {
  ...
  const upstreamCert = (await import('../src/main/proxy-tls.js')).generateHostCert('handler-test.local', ca)
  const httpsUpstream = https.createServer({ cert: upstreamCert.cert, key: upstreamCert.key }, (_req, res) => {
    res.writeHead(200)
...
await test('stop() cleans up all connections', async () => {
  ...
  assert.ok(socket.destroyed || socket.readableEnded || !socket.writable, 'socket should be closed after stop()')
  holder.close()
})
