...
const mockUpstream = https.createServer({ cert: upstreamCert.cert, key: upstreamCert.key }, (req, res) => {
  const url = req.url ?? '/'
...
const mockGitUpstream = https.createServer({ cert: gitUpstreamCert.cert, key: gitUpstreamCert.key }, (req, res) => {
