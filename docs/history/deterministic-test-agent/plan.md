...
```typescript
function buildReport(policyId: string, sessionResults: SessionReplayResult[], datasetPath: string): ReplayReport {
...
async function handleCreate() {
  ...
  setCreating(true)
  try {
    const session = await window.glitterball.sessions.create(projectDir, agentType, selectedPolicyId)
    onCreated(session)
