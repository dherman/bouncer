...
```typescript
import { parseApiEndpoint, evaluateApiPulls, evaluateApiIssues, type ApiEndpointMatch } from './github-policy-engine.js'
```
...
async function handleMitmRequest(
  ...
  if (hostname === 'api.github.com' && config.githubPolicy) {
    const decision = evaluateGitHubRequest(req.method ?? 'GET', req.url ?? '/', config.githubPolicy)
