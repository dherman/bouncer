# How to Understand Claude Code Session History
...
**Detecting Denied Tool Uses**:
When the user rejects a tool use at the permission prompt, the tool_result record has:

- **`toolUseResult`** (top-level): the string `"User rejected tool use"`
- **`message.content[].is_error`**: `true`
- **`message.content[].content`**: a string beginning with `"The user doesn't want to proceed with this tool use. The tool use was rejected..."`

Denied tool uses still produce a tool_result record — they are never silently omitted.

**Detecting Approved Tool Uses**:
There is no explicit field distinguishing a one-time "Yes" approval from a tool use that required no permission prompt at all (e.g., because the permission mode already allowed it). If a tool_result exists and is not a rejection, the tool was executed — but you cannot tell whether the user was prompted.
...
**"No, but do this instead"**:
When the user rejects a tool use and provides alternate instructions, the rejection appears as described above, followed by a separate user-type record with the user's text instructions in `message.content`. There is no structured link between the rejection and the follow-up instructions beyond adjacency (the follow-up's `parentUuid` will point to the assistant message that contained the rejected tool use).

**Tool Errors vs. Rejections**:
Not all `is_error: true` results are user rejections. System-level errors (e.g., `"File has not been read yet"`) also set `is_error: true` but have different content text and a descriptive `toolUseResult` string rather than `"User rejected tool use"`. Use the `toolUseResult` value to distinguish them.
